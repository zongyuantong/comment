...
public class PassageServiceImpl extends AbstractService<Passage> implements PassageService {
     ...
     *@param passageId
     */
    public void  deleteComments(String passageId){
        Condition condition  = new Condition(Comment.class);
        condition.createCriteria().andCondition("passage_id=", passageId);
        List<Comment> commentDetailList = commentMapper.selectByCondition(condition);

        for(int i = 0 ; i < commentDetailList.size(); i++){
            commentService.deleteComment(commentDetailList.get(i).getId());
        }
